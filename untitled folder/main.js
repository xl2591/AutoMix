...
window.onload = async () => {
  ...
  function playSound(context, isplayOriginal) {
    ...
    audioURL[audioDirIndex].forEach(async (url, key) => {
      ...
      if (isplayOriginal) {
        ...
        gainNode.connect(compressNodes[key], key, 0)//(peaqNode, key, key);
  ...
  async function training() {
    ...
    const context = new OfflineAudioContext(1, sampleRate * 20, sampleRate);
   ...
   // const context = new AudioContext();
    ...
    audioURL[audioDirIndex].forEach(async (url, key) => {
      ...
      context.decodeAudioData(arrayBuffer, (decodedData) => {
        ...
        // compressNodes[key].connect(peaqNode);
        // // panNodes[key].connect(peaqNode, 0, key);
        // peaqNode.connect(recordingNode[key], key, 0)
        // recordingNode[key].connect(totalRecordingNode);
    ...
    peaqNode.port.onmessage = (event) => {
      ...
      let peaqBuffer = audioData.value;
      //bufferMn[peaqIndex].push(peaqBuffer);
      bufferMn[peaqIndex].push(peaqValue);
      TotalM[peaqIndex] += peaqValue;

      temp[peaqIndex].push(peaqValue)
      TotalM[peaqIndex] += peaqValue;
      peaqFrameIdx[peaqIndex]++;

      //console.log(TotalM);
     //console.log(peaqBuffer);

     if (peaqFrameIdx[peaqIndex] == 1) {
      temp2[peaqIndex] = audioData.info
      console.log(peaqIndex)
      console.log("??")
    }
