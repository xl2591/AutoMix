...
class PEAQ extends AudioWorkletProcessor {
  ...
  constructor(options) {
    ...
    this.inputChanelSize = 1;
    this.outputSize = options.numberOfOutputs;
    this.outChanelSize = 1;
    ...
    for (let i = 0; i < this.inputSize; i++) {
      ...
      for (let j = 0; j < this.inputChanelSize; j++) {
   ...
   this.bufferOutput = new Array(this.outputSize);
    for (let i = 0; i < this.outputSize; i++) {
      ...
      for (let j = 0; j < this.outChanelSize; j++) {
  ...
  process(inputs, outputs, parameters) {
    ...
    for (var i = 0; i < inputs.length; i++) {
      if (inputs[i].length > 0&&inputs[i][0] != undefined) {

        var pcmData = new Array(inputs[i][0].length)
        for (let j = 0; j < inputs[i][0].length; j++) {
          // 填充 PCM 数据，例如 16 位 PCM 数据在 -32768 到 32767 之间
         //pcmData[j] = Math.floor(inputs[i][0][j] * 32768);
         pcmData[j] = inputs[i][0][j] * 32768;
        }
        ...
        this.bufferInput[i][0] = this.array32Concat(this.bufferInput[i][0], pcmData)
      }
    }
    ...
    if (bufferOk) {
      ...
      for (var i = 0; i < inputs.length; i++) {
        ...
        // peaq
        var accRes = this.peaqWork(accomanpySum);
        maskRes[i] = accRes.mask
    
        energy[i] = this.peaqWork(this.bufferInput[i][0]).ef;
        ...
        for (var j = 0; j < this.Nc; j++) {
          var result = 0;
          if (energy[i][j] > this.TOF && maskRes[i][j] > this.TOF) {
            result = 10 * Math.log10(accRes.ef[j] / energy[i][j]);
          }
          MSR.push(result)
        }
        ...
        this.port.postMessage({ "key": i, "mask": maskRes[i], "energy": energy[i], "Mn": Mn, "info":inputs[i][0] });
      }
    ...
    for (var i = 0; i < inputs.length; i++) {
      ...
      if (inputs[i] == undefined || inputs[i][0] == 0 || outputs[i]) {
        continue
      }
