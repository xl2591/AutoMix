...
class PEAQ extends AudioWorkletProcessor {
  ...
  constructor(options) {
    super(options);
    // importScripts('adaEq.js', 'PEAQ.js');
    this.cnt = 1;
    // Init
    this.inputSize = options.numberOfInputs;
    this.inputChanelSize = options.inChannelCount;s
  ...
  //   var result = 0;
  //   for (var i = 0; i < this.Nc; i++) {
  //     result += this.Sdb(i, l, E)
  //   }

  //   return Math.pow(10, result * 0.1)
  // }
 ...
 //dz就是delta 不确定要不要加
  ...
  process(inputs, outputs, parameters) {
    ...
    // process inputbuffer
    this.bufferInput[0][0] = this.array32Concat(this.bufferInput[0][0], inputs[0][0])
    if (this.bufferInput[0][0].length >= this.NF) {

      // convert db to x, and ifft
      // var maskDb = this.peaqWork(this.bufferInput[0][0]);
      // console.log(maskDb)
      // var mask = maskDb.real.map((val, i) => {return 0.00002 * Math.pow(10, val / 20)}), 
      //             'imag': maskDb.imag.map((val, i) => {return 0.00002 * Math.pow(10, val / 20)})};
      // var xMask = this.fftutil.ifft(mask).real

      // test
      // this.testComare(this.bufferInput[0][0].slice(0, this.NF), this.peaqWork(this.bufferInput[0][0]))

      this.peaqWork(this.bufferInput[0][0])
      //  this.bufferOutput[0][0] = this.array32Concat(this.bufferOutput[0][0], xMask)
      this.bufferInput[0][0] = this.bufferInput[0][0].slice(this.NF)
    }

    // output
    if (this.bufferOutput[0][0].length >= this.blockSize) {
      for (var i = 0; i < outputs[0][0].length; i++) {
        outputs[0][0][i] = this.bufferOutput[0][0][i];
      }
      this.bufferOutput[0][0] = this.bufferOutput[0][0].slice(this.blockSize);
    }
