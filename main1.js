...
//  const qValues = [1, 0.6, 0.6, 0.3, 0.3, 0.2,1,1];
 ...
 const biquadFilterFreqency = [75, 100, 250, 750, 2500, 7500];
  ...
  async function training() {
    ...
    const context = new OfflineAudioContext(1, sampleRate * 5, sampleRate);
   ...
   // const context = new AudioContext();
    ...
    const EqParameterRange = { min: -20, max: 20 }; // eq
    const DRCParameterRangeThreshold = { min: -30, max: 0 }; // drc.threshold
    const DRCParameterRangeRatio = { min: 1, max:10 }; // 参数范围
    const DRCParameterRangeAttack = { min: 0.005, max: 0.25 }; // 参数范围
    const DRCParameterRangeRelease = { min: 0.005, max: 1 }; // 参数范围

 if(iterTime == 0){
  for (var i = 0; i < inputSize; i++) {
  compressNodes[i].threshold.value =0;
  compressNodes[i].ratio.value = 1;
  compressNodes[i].attack.value = 0;
  compressNodes[i].release.value = 0;

  // set pan param
  //console.log(panNodes[i])
  panNodes[i].positionX.value = 0;
  panNodes[i].positionY.value = 0;
  panNodes[i].positionZ.value = 0;

  // set gain param
  //console.log(peakFilters[i])

  for (var j = 0; j < 6; j++) {
    peakFilters[i][j].gain.value = 0
  }
  }
 }else{
    for (var i = 0; i < inputSize; i++) {
      ...
      compressNodes[i].release.value = getRandom(DRCParameterRangeRelease.min, DRCParameterRangeRelease.max,0.001)
    ...
    var mergerNodes = new Array();
    for (var i = 0; i < inputSize; i++) {
      mergerNodes.push(context.createChannelMerger(6));
    }

    await audioURL[audioDirIndex].forEach(async (url, key) => {
    // audioURL[audioDirIndex].forEach(async (url, key) => {
      ...
      // context.decodeAudioData(arrayBuffer, (decodedData) => {
      //   bufferedSource.buffer = decodedData;
      //   bufferedSource.connect(gainNode, 0, key);
    
      //   for (var i = 0; i < peakFilters[key].length; i++) {
      //     gainNode.connect(peakFilters[key][i], key, 0)
      //     peakFilters[key][i].connect(mergerNodes[key], 0, i)
      //   }
      //   mergerNodes[key].connect(panNodes[key])
      //   panNodes[key].connect(peaqNode, 0, key)

      //   peaqNode.connect(totalRecordingNode);
      //   totalRecordingNode.connect(context.destination)
      ...
      context.decodeAudioData(arrayBuffer, (decodedData) => {
       ...
       // bufferedSource.connect(compressNodes[key])
       bufferedSource.connect(gainNode, 0, key);
       gainNode.connect(compressNodes[key], key, 0)
        //   for (var i = 0; i < peakFilters[key].length; i++) {
        //     compressNodes.connect(peakFilters[key][i], key, 0)
        //   peakFilters[key][i].connect(mergerNodes[key], 0, i)
        // }
        for (var i = 0; i < peakFilters[key].length; i++) {
          (compressNodes[key]).connect(peakFilters[key][i])
    ...
    //       bufferedSource.connect(gainNode, 0, key);
    //      gainNode.connect(compressNodes[key], key, 0);
    //     //bufferedSource.connect(peaqNode, 0, key);
    ...
    //     peaqNode.connect(recordingNode[key]);
    //     recordingNode[key].connect(totalRecordingNode);
    //     totalRecordingNode.connect(context.destination)
    ...
    peaqNode.port.onmessage = (event) => {
      ...
      // console.log()
      // console.log(peaqFrameIdx[peaqIndex])
 ...
 Math.seedrandom('random');
  ...
  function xcOpt(context, compressNodes, panNodes, peakFilters) {
    iterTime++;
    ...
    if (iterTime < iterCnt) {
      ...
      peaqFrameIdx = new Array(audioURL[audioDirIndex].length).fill(0);
      bufferMn =  new Array(audioURL[audioDirIndex].length);
